...
nucl_acid_ext_modify: Modified for small, calcarous organisms, reagent volume altered
dna_cleanup_0_1: 0
...
| Introduction to Developing DNA Reference Barcode Sequences |https://doi.org/10.5281/zenodo.14867762 | 1.0.0  | 2025-02-19  | External |
...
This document describes the standard protocol to subsample and extract DNA from organismal tissue. 
...
For more detail on organismal collection, see [Introduction to Developing DNA Reference Barcode Sequences] (https://doi.org/10.5281/zenodo.14867762).
...
*  For best quality photo, fully submerge the specimen in seawater, place black felt under the bin, and elevate using falcon tube caps. Then use a DSLR and flash for best image.
...
#### Tissue Subsampling
Ideal genetic subsamples are a couple of cubic millimeters, in ~1 mm sized pieces and are taken when organisms are sampled in the field. Once the organism is subsampled, place the tissue in a 1.5 mL tube and ensure you use ~10x the volume of fixative. With small animals or those that deteriorate rapidly (sponges, flatworms, etc.) it is important to fix a subsample before, or quickly after death as DNA degrades rapidly. If the time or setting doesn't allow field subsampling, this process can occur back in the laboraroty for less sensitive specimen.

1.  Sterilize benches and durable equipment by wiping with 10% bleach followed with 70% EtOH. For extra caution, sterilize forceps used for subsampling by dipping in 70% EtOH and flame sterilizing. 
2.  Place the sample on a flat surface or dissection tray then use sterilize equipment to subsample organism - see guidance below for each taxon.
...
* **Stony corals and hydrocorals:** Small colony fragments from tissue scrape works for subsample.
* **Flatworms:** Specimen photos are necessary for ID; take snippet of exterior end for genetic work
* **Worms:** Easier to photograph if relaxed via refrigeration; take a snippet of abdominal, posterior, or anterior muscular tissue for genetic subsample.
...
Full organismal samples stored in 70% EtOH can be stored in a dry storage room at room temperature. Tissue samples should be stored in -20°C freezer until extracted.
...
1. Sterilize benches and durable equipment by wiping with 10% bleach followed with 70% EtOH. 
2. For extra caution, sterilize forceps used for subsampling by dipping in 70% EtOH and flame sterilizing. 
3. Turn on the heatshaker to 56°C. If the Buffer ATL or AL have precipitate, place in heatshaker. Allow the buffer to heat until all precipitate is gone (~15 minutes), swirl as needed.
...
If the subsample is too large (> ~1 mm), use a sterilzied razor blade and forceps to break off a small piece and relocate it to a new 1.5 mL tube. Cutting the tissue into smaller pieces has been found to result in more efficient lysis. If you are dealing with a hard shelled organisms (e.g., gastropod), use a sterilized mortar and pestle to break down the hard outer shell and access the soft tissue for extraction. 

### Tissue Extraction
For most organismal samples, the standard QIAGEN Blood & Tissue protocol (Option A) as followed works for extracting DNA. However, for some smaller specimen (e.g., larval stage organisms, eggs) and/or organisms with a calcarious outer shell (e.g., pteropods, orphurioids), a modified protocol (Option B) with smaller volumes is more effective.
...
3. Incubate in heatshaker at 56°C until tissue is completely lysed. Lysis time varies depending on the type of tissue but is typically completed in 1-3 hours.

**Note**: Lysate may appear viscious but should not be gelatinous as it may clog the DNeasy Mini spin column. 
...
**Note**: A white precipitate may form on addition of Buffer AL and EtOH. This will not disrupt the DNeasy procedure. 
...
7. Centrifuge at ≥6000 x g (8000 rpm) for 1 min. Discard flow-through and collection tube. 
9. Place the DNeasy Mini spin column into a new 2 mL collection tube and add 500 μL Buffer AW1.
9. Centrifuge at ≥6000 x g (8000 rpm) for 1 min. Discard flow-through and collection tube. 
...
13. Place the DNeasy Mini spin column in a clean, labellled 1.5 mL tube. 
...
15. Store extracted DNA tubes in -20℃ freezer until later use.
